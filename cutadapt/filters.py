...
class VBIMRedirector(object):
    """
    Redirect trimmed VBIM reads to the given writer. This is for single-end reads.
    """
    ...
    def __call__(self, read):
        ...
        if r:
            ...
            if self.writer is not None:
                self.writer.write(r)
                self.written += 1
                self.written_bp[0] += len(r)
            return DISCARD
        return KEEP


class PairedVBIMRedirector(object):
    """
    Redirect trimmed VBIM reads to the writer
    """
    ...
    def __call__(self, read1, read2):
        ...
        if r1 or r2:
            ...
            if self.writer is not None:
                ...
                self.written += 1
                self.written_bp[0] += len(r1)
                self.written_bp[1] += len(r2)
            return DISCARD
        return KEEP
...
class VBIMSequenceFilter(object):
    def __init__(self, vbimseq):
        self.vbimseq = vbimseq
